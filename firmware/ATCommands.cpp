...
{
    uint16_t pos = 2;
    uint8_t type=AT_COMMAND_INVALID;
    ...
    if (!this->bufferString.startsWith("at") && !this->bufferString.startsWith("st") &&!this->bufferString.startsWith("AT"))
    {
        return false;
    }
    ...
    for (uint16_t i = 2; i < this->bufferSize; i++)
    {
        ...
        if (this->bufferString[i] == '=' || this->bufferString[i] == ' ')
        {
...
{
    return (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || (ch == '+') || (ch == '#') || (ch == '$') || (ch == '@') || (ch == '_') || (ch == '=') || (ch == '?') || (ch == ' ');
}
